...
class LearningDynamicsModel(object):
    ...
    def sample(self, T, model_params, num_obs_samples=10, dim=3, x=None, switching=False, alpha_time_dep=False):
        '''
        ...
        for i in range(1, T):
            ...
            z.append(self.sample_prior(model_params, z[i-1], y[i-1], x[i-1], switching, model_params['log_alpha'][i-1]))
    ...
    def sample_gaussian_random_walk(self, T, init_mean, init_log_scale, transition_log_scale, decay):
        #x = torch.tensor(np.arange(-3, -1, step=.002)[:-1], dtype=dtype, device=device)
        init_mean = torch.tensor([-3.], dtype=dtype, device=device)
        init_dist = Normal(init_mean, torch.exp(init_log_scale))
        x = [init_dist.sample()]
        for i in range(1,T):
            dist = Normal(x[i-1], torch.exp(transition_log_scale))
            x.append(dist.sample())
        return torch.stack(x)


    def sample_prior(self, model_params, z_prev, y_prev, x_prev, switching, log_alpha):
        ...
        if switching:
            ...
            mean = z_prev - torch.exp(log_alpha) * grad_loss
...
## vectorized functions ##
    def log_joint(self, model_params, y, x, z, log_alpha=None):
        '''
        ...
        logprob += self.log_prior_vec(model_params, z, y, x, log_alpha)
    ...
    def log_prior_vec(self, model_params, z, y, x, log_alpha, switching=False):
        '''
            input: z_1:t
        ...
        prior_log_alpha = Normal(model_params['log_alpha'][0:-1], torch.exp(model_params['alpha_log_diffusion_prior']))
        init_prior_log_alpha = Normal(model_params['log_alpha_init_latent_loc'], torch.exp(model_params['log_alpha_init_latent_log_scale']))

        return torch.sum(prior.log_prob(z_curr)) + torch.sum(prior_scale.log_prob(model_params['transition_log_scale'])) + \
            torch.sum(prior_log_alpha.log_prob(model_params['log_alpha'][1:])) + torch.sum(init_prior_log_alpha.log_prob(model_params['log_alpha'][0]))
    
    def log_prior_vecNOALPHA(self, model_params, z, y, x, switching=True):
        '''
            input: z_1:t
            parameterize p(z_t | z_t-1, theta)
        '''
