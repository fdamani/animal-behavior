...
class Inference(object):
    def __init__(self, data, model, savedir, num_obs):
        ...
        self.var_params = self.vi.init_var_params(self.dim)
...
class MeanFieldVI(object):
    ...
    def init_var_params(self, dim):
        mean = torch.ones(dim, requires_grad=True, device=device)
        log_scale = -5 * torch.ones(dim, requires_grad=True, device=device)
