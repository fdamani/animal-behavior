...
class Inference(object):
    def __init__(self, 
        ...
        alpha_map = self.alpha_map_estimate(self.init_z)
        embed()
        ...
        #self.opt_params['log_alpha_var_log_scale'] = self.model_params['log_alpha_var_log_scale']
    ...
    def map_estimate(self):
        ...
        for t in range(self.map_iters):
            def closure():
                ...
                output = -self.model.log_joint(self.model_params, y, x, z, self.model_params['log_alpha'])
                output.backward()
                return output
            if lbfgs:
    ...
    def alpha_map_estimate(self, map_z):
        # initialize to all ones = smooth.
        z = map_z
        #z = torch.tensor(torch.rand(self.T, self.dim, dtype=dtype, device=device), requires_grad=True, dtype=dtype, device=device)
        y, x = self.unpack_data(self.data)
        self.map_iters = 100
        self.opt_params = [self.model_params['log_alpha']]
        #self.map_optimizer =  torch.optim.Adam(self.opt_params, lr=1e-3)
        self.map_optimizer = torch.optim.LBFGS(self.opt_params)
        lbfgs = True
        for t in range(self.map_iters):
            ...
            if t % 5 == 0:
                plt.cla()
                plt.plot(np.exp(to_numpy(self.model_params['log_alpha'])))
                figure = plt.gcf() # get current figure
                figure.set_size_inches(8, 6)
                plt.savefig(self.savedir+'/plots/curr_map_alpha.png')
        return self.opt_params[0].clone().detach()
    ...
    def optimize(self, iters, lbfgs, print_every):
        ...
        for t in range(self.iters):
            ...
            #output = -self.vi.forward_with_time_dep_alpha(self.model_params, self.train_data, self.var_params, t) #/ float(self.num_train)
            ...
            if t % print_every == 0:
                ...
                if 'log_alpha_var_log_scale' in self.opt_params:
                    print torch.mean(self.opt_params['log_alpha_var_log_scale'].clone().detach()).item(),
                if 'model_mu' in self.opt_params:
                    print self.opt_params['model_mu'].item(), self.opt_params['model_log_scale'].item()
                for k,v in self.model_params_grad.items():
                    if k == 'log_alpha': continue
                    if v == True:
                        if k in self.opt_params:
                ...
                plt.cla()
                plt.plot(np.exp(to_numpy(self.model_params['log_alpha'])))
                plt.savefig(self.savedir+'/plots/curr_est_log_alpha.png')
...
class MeanFieldVI(object):
    ...
    def forward_with_time_dep_alpha(self, model_params, data, var_params, itr, num_samples=1):
        '''
            useful for analytic kl  kl = torch.distributions.kl.kl_divergence(z_dist, self.prior).sum(-1)
        '''
        y, x = self.unpack_data(data)
        loc, log_scale = self.unpack_var_params(var_params)
        alpha_loc, alpha_log_scale = model_params['log_alpha'], model_params['log_alpha_var_log_scale']
        var_dist = Normal(loc, torch.exp(log_scale))
        alpha_var_dist = Normal(alpha_loc, torch.exp(alpha_log_scale))
        #cov = torch.diag(torch.exp(log_scale))**2
        #scale_tril = cov.tril()
        #var_dist = MultivariateNormal(loc, scale_tril=scale_tril)
        samples = var_dist.rsample(torch.Size((num_samples,)))
        log_alpha_samples = alpha_var_dist.rsample(torch.Size((num_samples,)))
        data_term = self.model.log_joint(model_params, y, x, samples[0], log_alpha_samples[0])
        entropy = torch.sum(var_dist.entropy())
        return (data_term + entropy)
